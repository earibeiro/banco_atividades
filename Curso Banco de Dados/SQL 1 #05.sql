...
-- SELECT @@version, @version_compile_os, @@lower_case_table_names; -- aqui ele vai deixar o nome das tabelas sempre minusculo, independentemente do que for escrito ou como for escrito
-- No linux os comandos e tabelas são POR PADRÃO CASE SENSITIVE, então se você errar uma letra ali, vai dar errado.

-- Em linguagem de programação é comum usar o padrão categoriaProduto
-- Em BD não se utiliza o padrão acima, pois geralmente fica tudo minúsculo e não vai ser possível distinguir as duas palavras
